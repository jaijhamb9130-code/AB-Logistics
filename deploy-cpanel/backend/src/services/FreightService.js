 ...
 * Stores computed totals at generation time (snapshot). Re-run
 * `regenerateMemo` to recompute against the latest bilty data.
 */
 ...
 * Last-write-wins; OCC removed by user request.
 * @throws {{ code: 'memo_not_found' }}
 */
async function regenerateMemo({ memoId, userId, ip }) {
  return transact(pool, async (conn, audit) => {
