...
const SYSTEM_GROUP_IDS = new Set([1, 2, 3]);
...
exports.create = async (req, res, next) => {
  try {
    ...
    if (err && err.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({ error: 'name_taken', message: 'A group with that name already exists.' });
    }
    return next(err);
  }
};
...
exports.remove = async (req, res, next) => {
  try {
    ...
    if (SYSTEM_GROUP_IDS.has(id)) {
      return res.status(409).json({
        error: 'system_group_locked',
        message: 'Party / Owner / Agent are system groups and cannot be deleted.',
      });
    }
