...
async function _lookupLedgerIdByName(conn, name) {
  const [rows] = await conn.execute(
    'SELECT id FROM party_ledger WHERE name = :name LIMIT 1',
    { name }
  );
...
async function _insertChildEntries(conn, vchId, data) {
  ...
  const partyId   = data.party_ledger_id;
  ...
  for (const item of items) {
    ...
    if (item.batch_rows && item.batch_rows.length > 0) {
      for (const b of item.batch_rows) {
        await conn.execute(
          `INSERT INTO batch (vch_id, inventory_id, item_id, batch_name, qty, rate, amount)
           VALUES (:vchId, :invId, :itemId, :batchName, :qty, :rate, :amount)`,
          {
  ...
  return pool.withTransaction(async (conn) => {
    ...
    const [r] = await conn.execute(
      `INSERT INTO vch_details (vch_type_id, vch_no, vch_date, party_ledger_id, amount, remark, created_by)
       VALUES (:vchTypeId, :vchNo, :vchDate, :partyLedgerId, :amount, :remark, :userId)`,
      {
        vchTypeId: data.vch_type_id || null,
        vchNo: data.vch_no || null,
        vchDate: data.vch_date || null,
        partyLedgerId: data.party_ledger_id,
        amount: headerAmount,
    ...
    await conn.execute(
      `UPDATE vch_details
         SET vch_type_id = :vchTypeId, vch_no = :vchNo, vch_date = :vchDate,
             party_ledger_id = :partyLedgerId, amount = :amount, remark = :remark
       WHERE id = :id`,
      {
        id,
        vchTypeId: data.vch_type_id || null,
...
async function findById(id) {
  const [headerRows] = await pool.execute(
    `SELECT v.*, pl.name AS party_name,
     ...
     FROM vch_details v
     LEFT JOIN party_ledger pl ON v.party_ledger_id = pl.id
  ...
  const [ledgerEntries] = await pool.execute(
    `SELECT le.id, le.vch_id, le.ledger_id, le.amount, le.created_at,
     ...
     LEFT JOIN party_ledger pl ON le.ledger_id = pl.id
  ...
  for (const le of ledgerEntries) {
    ...
    for (const ie of invs) {
      const [batches] = await pool.execute(
        'SELECT id, batch_name, qty, rate, amount FROM batch WHERE inventory_id = :invId ORDER BY id',
        { invId: ie.id }
      );
